...
export function WaterGoalsChart({ data }: { data: { week: string; count: number }[] }) {
  ...
  if (chartData.length === 0) {
    return <p className="py-8 text-center text-sm text-pulse-muted">No water goals hit yet.</p>;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={chartData}>
        ...
        />
        <Bar dataKey="count" fill="url(#waterGradient)" radius={[6, 6, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}
